#!/usr/bin/env bash
# Stage 2-A 全流程驱动：A1(LoRA+大lr) 与 A2(全参) 串行跑完并各自评测。
...
echo "########## Stage 2-A 开始 $(date '+%F %T') ##########"
run_one s2a1_lora_lr1e-5 configs/grpo_a1_lr.yaml  lora
run_one s2a2_full_lr2e-6 configs/grpo_a2_full.yaml full

echo
echo "########## 配对显著性检验 ##########"
source /etc/profile.d/conda.sh && conda activate rlpt
for t in eval_s2a1_lora_lr1e-5 eval_s2a2_full_lr2e-6; do
  if [ -f "$LOG/$t.jsonl" ]; then
    echo; echo "### baseline vs $t"
    python src/compare.py --before baseline_base_1.5b --after "$t"
  fi
done
echo "########## Stage 2-A 结束 $(date '+%F %T') ##########"
